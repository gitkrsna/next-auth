...
const AddTimeTable = ({ isEditing = false, initialValues = {
    id: "",
    start_time: "",
    end_time: "",
    day_of_week: "",
    ...
    teacher_id: "",
    created_at: null,
}, refreshTimeTables }: AddTimeTableProps) => {
    ...
    async function onSubmit(values: FieldValues) {
        ...
        if (isEditing) {
            const { error: timeTableUpdateFailed } = await supabase.from('timetable').update(values).eq("id", values.id)
            ...
            const id = v4();
            const { error: timetableCreationFailed } = await supabase.from('timetable').insert({
                ...values,
                id
            }).select('*')
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>{isEditing ? <DropdownMenuItem onSelect={(e) => e.preventDefault()} >Edit</DropdownMenuItem> : <Button className="ml-5">Add TimeTable  <PlusIcon className="ml-2 h-4 w-4" /></Button>}</DialogTrigger>
            <DialogContent>
                <DialogHeader>
