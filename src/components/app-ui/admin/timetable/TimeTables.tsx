...
export function TimeTables() {
  ...
  const fetchTimeTables = async () => {
    ...
    const { data, error } = await supabase
      .from("timetable")
      .select(
        "id, class_id, subject_id, teacher_id, start_time, end_time, day_of_week, classes(*), teacher(id, user(id, first_name, last_name)), subjects(*), created_at",
      );
  ...
  const columns = useTimeTableColumns({
  ...
  return (
    <GenTable
      ...
      data={timeTables}
      searchId="Class name"
      rightComponent={<AddTimeTable refreshTimeTables={fetchTimeTables} />}
    />
  );
}
